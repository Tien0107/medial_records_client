...
function HomePage() {
  ...
  useEffect(() => {
    if (isDoctor) {
      window.location.href = 'https://react-tsx-export-pdf.vercel.app';
    }
  }, [isDoctor]);
  ...
  const handleAddRecord = async () => {
    try {
      setUploadStatus('Đang thực hiện transaction...');
      const autoPatientID = Date.now() + Math.floor(Math.random() * 1000);
      const success = await Healthcare.addRecord(
        autoPatientID,
      ...
      if (success) {
        ...
        setUploadStatus('Lỗi: Không thể thêm hồ sơ');
      }
    } catch (error) {
        ...
        {!account ? (
          ...
          <div>
            ...
            <div className="mb-6">
              ...
              >
                <option value="patient" className='text-black'>Bệnh nhân</option>
                <option value="doctor" className='text-black'>Bác sĩ</option>
              </select>
            </div>
            ...
            <div className="bg-green-50 p-8 rounded-xl shadow-2xl max-w-md mx-auto mt-10">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-base font-medium text-gray-800 mb-1">Điều trị</label>
                  <textarea
                    name="treatment"
